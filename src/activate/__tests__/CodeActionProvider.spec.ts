...
vi.mock("../../i18n", () => ({
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"common:codeActions.explain": "Explain with Trinit",
			"common:codeActions.fix": "Fix with Trinit",
			"common:codeActions.improve": "Improve with Trinit",
			"common:codeActions.addToContext": "Add to Trinit",
		}
...
describe("CodeActionProvider", () => {
	...
	describe("provideCodeActions", () => {
		it("should provide explain, improve, fix logic, and add to context actions by default", () => {
			...
			expect((actions as any)[0].title).toBe("Add to Trinit")
			expect((actions as any)[1].title).toBe("Explain with Trinit")
			expect((actions as any)[2].title).toBe("Improve with Trinit")
		})
		...
		it("should provide fix action instead of fix logic when diagnostics exist", () => {
			...
			expect((actions as any).some((a: any) => a.title === "Fix with Trinit")).toBe(true)
			expect((actions as any).some((a: any) => a.title === "Add to Trinit")).toBe(true)
		})
		...
		it("should return empty array when enableCodeActions is disabled", () => {
			...
			expect(actions).toEqual([])
			expect(vscode.workspace.getConfiguration).toHaveBeenCalledWith("trinit")
